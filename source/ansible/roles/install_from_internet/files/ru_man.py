...
def recursive_chown(path, user, group, rwx_dirs: str, rwx_files: str):
    for dir_path, dir_names, file_names in os.walk(path):
        shutil.chown(dir_path, user, group)
        os.chmod(dir_path, int(rwx_dirs, base=8))
        for file_name in file_names:
            file = os.path.join(dir_path, file_name)
            shutil.chown(file, user, group)
            os.chmod(file, int(rwx_files, base=8))
...
if __name__ == "__main__":
    ...
    unpack_dir = os.path.join(os.path.dirname(file), 'man_ru')
    extract_bz2(file, unpack_dir)
    # например, man_dir = /tmp/man/man-pages-ru_5.03-2390-2390-20191017
    man_dir = os.path.join(unpack_dir, os.listdir(unpack_dir)[0])
    ...
    recursive_chown(man_dir, 'root', 'root', rwx_dirs='755', rwx_files='644')
